...
- 视觉升级为“沉浸星球主页 × 极简工具页”双域反差：主页是深空仪式感（3D 自转星球、星尘、星云、大气脉冲），工具页是克制的实验室工作台。
- 主页重建 3D 双星球：球面经纬纹理按三维坐标投影并横向自转，前后遮挡的连续星轨、大气呼吸脉冲、星尘与星云底景；专注（青）/休息（紫）双色相，并尊重系统“减弱动画”设置。
- 监测页重构为“由简入繁”：未连接时全屏只留一个扫描按钮，连接后整页沉浸式单画布脑电波形，复杂控制（视图/通道/采集/阻抗/指标/数据源）收进底部控制面板。
- 图标体系从 Unicode 全面替换为 Material Icons，修复符号族混杂、语义错配与无障碍（TalkBack）不可读问题。
- 全应用主题 token 化：语义色、统一圆角节奏、动效时长集中到单一主题文件；坐标轴标签 ≥11sp 并随系统字体缩放。
- 既有功能（在线音频、自动更新、BLE 扫描、CSV 导出、8 通道分析、模块页、阻抗估算、本地账号）全部保留。
